...
for signifier in ["kernel", "s4_layers", "coder", "output_linear", "norms", "all"]:
    if signifier == "kernel":
        specList = ["A_real", "A_imag", "B", "C", "P", "inv_dt"]
    else:
        specList = ["weight", "bias", "all"]
    for spec in specList:
        acc_file = open(results_folder + "acc_redPrec_" + signifier + "_" + spec + ".txt", "w")
        for i in range(1, 17, 1):
            acc_file = open(results_folder + "acc_redPrec_" + signifier + "_" + spec + ".txt", "a+")
            red_prec = 2**i
            
            # Load checkpoint.
            print('Loading checkpoint..')
            assert os.path.isdir('./checkpoint'), 'Error: no checkpoint directory found!'
            checkpoint = torch.load('./checkpoint/quant_baseline.pth')
            checkpoint_red = checkpoint

            for layer in checkpoint['model']:
                if signifier in layer or signifier == "all":
                    if spec in layer or spec == "all":
                        checkpoint_red['model'][layer] = pt.reduce_precision_t(checkpoint['model'][layer], red_prec)

            model.load_state_dict(checkpoint_red['model'])

            acc = eval(0, testloader)
            acc_file.write(format(i) + "\t" + format(acc) + "\n")
            acc_file.close()


checkpoint_base = torch.load('./checkpoint/quant_baseline.pth')
model.load_state_dict(checkpoint_base['model'])
base_acc = eval(0, testloader)
