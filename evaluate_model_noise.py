...
def eval(model, dataloader, save_path=None, num_batches=None, noise_mag=None, noise_std=None):
    global best_acc
    model.eval()
    correct = 0
    total = 0
    ...
    '''if save_path is not None:
        ...
        try:
            ...
            pass
    file_encoder = open(save_path + "/act_encoder_quant.txt", "a+")
    file_decoder = open(save_path + "/act_decoder_quant.txt", "a+")
    file_layers = []
    for i in range(len(model.s4_layers)):
        file_layers.append(open(save_path + "/act_l" + format(i) + "_quant.txt", "a+"))'''
    ...
    with torch.no_grad():
        ...
        for batch_idx, (inputs, targets) in pbar:
            ...
            inputs = inputs + 0.5 * torch.normal(0., 0.5, inputs.shape)

            outputs, y_encoder, y_layers = model(inputs)
            ...
            f#or i, layer in enumerate(y_layers):
...
def evaluate_model(check_point, model_args, save_path=None, num_batches=None):
    ...
    base_acc = eval(model_test, testloader, save_path=save_path, num_batches=num_batches)
    print("Test accuracy:\t" + format(base_acc))


evaluate_model(checkpoint_test, model_args, save_path=save_path, num_batches=None)
