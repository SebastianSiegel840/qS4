...
class QuantizedLinear(Linear):
    r"""Works like standard linear layer, but implements quantization with a Straight Through Estimator for the update.
    ...
    Args:
        ...
        quant_fn: quantization function
    ...
    def forward(self, input: Tensor) -> Tensor:
        if self.quant_levels is not None:
            return F.linear(input, self.weight - (self.weight - self.extra_reprquant_fn(self.weight, self.quant_levels)), self.bias)
        else:
            return F.linear(input, self.weight, self.bias)
